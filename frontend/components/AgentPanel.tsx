...
export default function AgentPanel({ running, onRun, onStop }: Props) {
  ...
  const cellClass = (i: number) =>
    `w-full h-full bg-white text-gray-800 text-sm font-mono px-3 py-2 placeholder-gray-300 focus:outline-none resize-none border rounded ${
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="flex items-center gap-4">
        ...
        <select value={lang} onChange={e => setLang(e.target.value)}
          className="ml-auto bg-white border border-gray-300 text-gray-800 text-sm rounded px-2 py-1.5">
      ...
      {mode === 'structured' ? (
        <div className="flex flex-col gap-1">
          <div className="grid grid-cols-[2fr_1fr] gap-2" style={{ height: '180px' }}>
            <textarea
              value={cells[0]}
      ...
      <div className="flex justify-end">
        {running
          ? <button onClick={onStop}
              className="px-4 py-2 bg-red-600 hover:bg-red-500 text-white text-sm rounded">Stop</button>
          : <button onClick={run}
              className="px-4 py-2 bg-blue-600 hover:bg-blue-500 text-white text-sm rounded">Run</button>
        }
      </div>
    </div>
  )
