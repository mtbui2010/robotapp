...
const CONNECTION_CONFIGS: { type: string; label: string; example: string }[] = [
  ...
  { type: 'ZeroMQ',      label: 'ZeroMQ (REQ client)', example: '{"host": "localhost", "port": 8888, "is_client": true, "timeout": 10000}' },
  { type: 'WebSocket',   label: 'WebSocket (client)', example: '{"host": "localhost", "port": 8888, "path": "/", "is_client": true}' },
  { type: 'HTTP/REST',   label: 'HTTP / REST (Inferix infer)', example: '{"is_client": true, "url": "https://label.aistations.org/api/deploy/endpoint-xxxx/infer_array", "token": "<INFERIX_TOKEN>", "timeout": 30}' },
  { type: 'VisionServe', label: 'VisionServe (detect)', example: '{"is_client": true, "url": "http://localhost:11435", "model": "rf-detr", "timeout": 30}' },
